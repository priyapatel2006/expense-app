...
function Addtransactions({ onClick, setTransactions }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        setTransactions(prev => [...prev, { ...data, type: data?.type || "expense" }])
        onClick()
    }
    ...
    return (
        ...
        <div className="fixed z-10 inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
            <form onSubmit={handleSubmit}>
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
                    ...
                    <div className="flex justify-end gap-3 p-6">
                        <button
                            onClick={onClick}
                            className="px-12 rounded-md py-2 text-md border border-gray-300 font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-100">
                            Cancel
                        </button>
                        <button className="px-6 py-2 bg-purple-800 text-white text-sm font-medium rounded-lg hover:bg-cyan-800 flex items-center gap-2">
